...
use crate::pages::rest::post_type_api::{load_post_type_by_id, update_post_type, PostTypeTO};
use leptos::prelude::*;
use leptos::{component, view, IntoView};
use leptos_router::hooks::use_params_map;

#[component]
pub fn AdminPostTypeEditPage() -> impl IntoView {
    let params = use_params_map();
    let id = move || params.with(|p| p.get("id").and_then(|v| v.parse::<i32>().ok())).unwrap_or(0);

    let entity_res = Resource::new(move || id(), |id| async move { load_post_type_by_id(id).await });
    ...
    let save_action = Action::new(move |(code_v, name_v): &(String, String)| {
        let mut current = entity_res.get().and_then(|r| r.ok()).unwrap_or(PostTypeTO{
            id: id(),
            uid: String::new(),
            version: 0,
        ...
        async move { update_post_type(current).await }
    });
                ...
                <div class="flex gap-0">
                    ...
                    <main class="flex-1 min-h-screen">
                        <div class="container-page py-10 font-serif">
                            <h1 class="text-3xl font-bold mb-6">"Edit Post Type"</h1>
