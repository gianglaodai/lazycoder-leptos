...
impl From<PostTermInfo> for PostTermInfoTO {
    fn from(entity: PostTermInfo) -> Self {
        Self {
            id: entity.id,
            uid: entity.uid,
            ...
            term_slug: entity.term_slug,
            term_name: entity.term_name,
...
pub async fn count(state: Data<AppState>, query: Query<QueryOptions>) -> impl Responder {
    respond_result(state.post_term_info_service.count(query.to_filters()).await)
}
