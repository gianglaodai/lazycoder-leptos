...
#[get("/count")]
pub async fn count_attributes(state: Data<AppState>, query: Query<QueryOptions>) -> impl Responder {
    respond_result(state.attribute_service.count(query.to_filters()).await)
}

#[get("/{id}")]
pub async fn get_attribute_by_id(state: Data<AppState>, id: Path<i32>) -> impl Responder {
    respond_result(
        state
            .attribute_service
...
#[get("/uid/{uid}")]
pub async fn get_attribute_by_uid(state: Data<AppState>, uid: Path<String>) -> impl Responder {
    respond_result(
        state
            .attribute_service
...
pub fn routes_attributes(cfg: &mut ServiceConfig) {
    cfg.service(
        scope("/api/attributes")
            .service(get_many_attributes)
            .service(count_attributes)
            .service(get_attribute_by_id)
            .service(get_attribute_by_uid),
    );
}
...
impl From<AttributeValueInfo> for AttributeValueInfoTO {
    fn from(entity: AttributeValueInfo) -> Self {
        Self {
            ...
            string_value: entity.string_value,
            ...
            attribute_entity_type: entity.attribute_entity_type,
            attribute_data_type: entity.attribute_data_type,
...
#[get("")]
pub async fn get_many_attribute_values(
    state: Data<AppState>,
    query: Query<QueryOptions>,
) -> impl Responder {
    respond_results(
...
#[get("/{id}")]
pub async fn get_attribute_value_by_id(state: Data<AppState>, id: Path<i32>) -> impl Responder {
    respond_result(
        state
            .attribute_value_info_service
            .get_by_id(id.into_inner())
...
#[get("/uid/{uid}")]
pub async fn get_attribute_value_by_uid(
    state: Data<AppState>,
    uid: Path<String>,
) -> impl Responder {
    respond_result(
...
pub fn routes_attribute_values(cfg: &mut ServiceConfig) {
    cfg.service(
        scope("/api/attribute-values")
            .service(get_many_attribute_values)
            .service(count_attribute_values)
            .service(get_attribute_value_by_id)
            .service(get_attribute_value_by_uid),
    );
}
